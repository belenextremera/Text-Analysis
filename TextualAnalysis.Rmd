...
```{r}
nivelbajo <- read_file("nivelbajopreseea.txt")
nivelbajo
...
```{r}
nivelbajocolumna = as.data.frame(nivelbajo)
nivelbajocolumna
```
...
```{r}
nivelbajolimpio <- nivelbajocolumna %>% 
    unnest_tokens(nivelbajocolumna, nivelbajo) %>%
  mutate(user="nivelbajo", Genero=1)
nivelbajolimpio %>%
  head
```
...
nivelbajolimpio <- 
nivelbajolimpio %>%
  rename(palabras=nivelbajocolumna)
```
...
```{r}
nivelbajopalabrastemáticas <-  anti_join(nivelbajolimpio, my_stopwords)
nivelbajopalabrastemáticas
```


### Tenemos: nivelbajopalabrastemáticas y nivelbajolimpio todas las palabras en filas
...
```{r}
(frecuencias <- count(nivelbajolimpio, palabras))
```

###6.2. Palabras temáticas

```{r}
frecuenciastemáticas <- count(nivelbajopalabrastemáticas, palabras) 
...
ggpalabrastemáticas <- 
  ...
  ggtitle("20 palabras temáticas más frecuentes en el nivel bajo") +
...
```{r}
palabrastextos <- tokenize_words(nivelbajo)
palabrastextos
```
...
```{r}
oraciones <- tokenize_sentences(nivelbajo)
oraciones
```


```{r}
texto_palabras <- get_tokens(nivelbajo)
texto_palabras
```


```{r}
sentimientos <- get_nrc_sentiment(nivelbajopalabrastemáticas$palabras, lang = "spanish") 
sentimientos
...
```{r}
Analisissentimientos <- cbind (nivelbajopalabrastemáticas, sentimientos) %>%
  unique
